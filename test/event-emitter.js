...
describe('eventEmitter.after()', function() {
	it('triggers event after flags are set', function(done) {
		var eventEmitter = new EventEmitter();
		eventEmitter._flags.foo = [];
		eventEmitter._flags.bar = [];
		eventEmitter.after(['foo', 'bar'], done);
	});

	it('triggers event after events are emitted', function(done) {
		var eventEmitter = new EventEmitter();
		eventEmitter.after(['foo', 'bar', 'baz'], done);
		eventEmitter.emit('bar');
		eventEmitter.emit('foo');
		eventEmitter.emit('baz');
	});

	it('handles a mix of flags and events', function(done) {
		var eventEmitter = new EventEmitter();
		eventEmitter._flags.foo = [];
		eventEmitter._flags.bar = [];
		eventEmitter.after(['foo', 'bar', 'baz'], done);
